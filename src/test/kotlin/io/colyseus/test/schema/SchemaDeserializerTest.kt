...
class SchemaDeserializerTest {
    ...
    fun `primitive types test`() {
        val state = io.colyseus.test.schema.primitive_types.PrimitiveTypes()
...
//        assertEquals(Mathf.Infinity, state.varint_float64)
    ...
    fun `child schema types test`() {
        val state = io.colyseus.test.schema.child_schema_types.ChildSchemaTypes()
    ...
    fun `array schema types test`() {
        val state = io.colyseus.test.schema.array_schema_types.ArraySchemaTypes()
    ...
    fun `map schema types test`() {
        val state = io.colyseus.test.schema.map_schema_types.MapSchemaTypes()
    ...
    fun `map schema int8 test`() {
        val state = io.colyseus.test.schema.map_schema_int8.MapSchemaInt8()
    ...
    fun `inherited types test`() {
        val serializer = SchemaSerializer(io.colyseus.test.schema.inherited_types.InheritedTypes::class.java)
    ...
    fun `backwards forwards test`() {
        ...
        val statev2 = io.colyseus.test.schema.backwards_forwards.StateV2()
        statev2.decode(statev1bytes)
        assertEquals("Hello world", statev2.str)

        val statev1 = io.colyseus.test.schema.backwards_forwards.StateV1()
    ...
    fun `filtered types test`() {
        val client1 = io.colyseus.test.schema.filtered_types.State()
        ...
        val client2 = io.colyseus.test.schema.filtered_types.State()
    ...
    @Test
    fun `instance sharing types test`() {
        val refs = ReferenceTracker()
        val client = io.colyseus.test.schema.instance_sharing_types.State()
